...
function badge(idx) {
	...
	req.open(
		"GET",
		"https://antoninformatik.net/badges/get/?name=" + WA.player.name
	);
	req.send();
}

/* Badge Eingang */
let badgeEingangPopup = WA.room.onEnterLayer("badgeEingang").subscribe(() => {
	badge(0);
});

/* Badge Aula */
let badgeAulaPopup = WA.room.onEnterLayer("badgeAula").subscribe(() => {
	badge(1);
});

/* Badge Physik */
let physikBadgePopup = WA.room
...
/* Badge Bibliothek */
let badgeBibliothekPopup = WA.room
...
/* Badge Informatik */
let badgeInformatikPopup = WA.room
...
/* Badge Klassenraum */
let badgeKlassenraumPopup = WA.room
